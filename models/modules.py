...
class DetailFeatureExtraction_(nn.Module):
    def __init__(self, num_layers=1, dim=2):
        ...
        inn_modules = [DetailNode() for _ in range(num_layers)]
        self.net = nn.Sequential(*inn_modules)
...
class DeformableConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1, modulation=True):
        ...
        self.offset_conv = nn.Conv2d(1, 2 * kernel_size * kernel_size, kernel_size=3, padding=1)
        if modulation:
